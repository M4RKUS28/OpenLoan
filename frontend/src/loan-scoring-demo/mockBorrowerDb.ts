import type { BorrowerScoreRecord } from "./types";
import { hashToken } from "./utils";

export const BORROWER_SCORE_RECORDS: Record<string, BorrowerScoreRecord> = {
  brw_001: {
    borrower_id: "brw_001",
    borrower_profile: {
...
export function getBorrowerScoreRecord(borrowerId: string): BorrowerScoreRecord {
  return BORROWER_SCORE_RECORDS[borrowerId] ?? buildDefaultThinFileBorrower(borrowerId);
}

function buildDefaultThinFileBorrower(borrowerId: string): BorrowerScoreRecord {
  const token = hashToken(`default-borrower|${borrowerId}`, 8).toUpperCase();

  return {
    borrower_id: borrowerId,
    borrower_profile: {
      legal_name: `Thin File Demo Borrower ${token}`,
      hong_kong_business_registration_number: `BR-${token}`,
